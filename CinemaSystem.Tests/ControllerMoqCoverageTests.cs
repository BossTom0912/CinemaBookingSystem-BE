...
public sealed class ControllerMoqCoverageTests
{
    ...
    public async Task Movies_GetMoviesAndDetail_ReturnSuccessAndNotFound()
    {
        ...
        var movies = AssertApiResponse<IReadOnlyList<MovieResponse>>(await controller.GetMovies("NOW_SHOWING", 1, 10, false, CancellationToken.None), StatusCodes.Status200OK, true);
    ...
    public async Task Rooms_AllEndpoints_ForwardRequestsAndMapStatuses()
    {
        ...
        service.Setup(x => x.GetRoomsAsync(It.IsAny<bool>(), It.IsAny<CancellationToken>()))
