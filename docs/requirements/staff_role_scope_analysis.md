...
4.  **Cửa sổ thời gian check-in (Time Window):**
    *   **Check-in sớm:** Khách chỉ được check-in trước giờ chiếu tối đa là `OpenBeforeStartMinutes` (thường cấu hình là 15 - 30 phút). Quét quá sớm -> Báo lỗi `CheckInTooEarly`.
    *   **Check-in muộn:** Khách chỉ được check-in muộn nhất là sau khi suất chiếu kết thúc `CloseAfterEndMinutes` phút. Quét quá trễ -> Báo lỗi `CheckInWindowClosed`.
...
*   **Sub-tasks:**
    ...
    2.  Cấu hình tham số thời gian mở/đóng cửa soát vé (`OpenBeforeStartMinutes` và `CloseAfterEndMinutes`) trong file `appsettings.json` thay vì hardcode.
