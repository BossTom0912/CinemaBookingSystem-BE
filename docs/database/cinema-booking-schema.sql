...
CREATE TABLE [BOOKING] (
    ...
    [compensationDiscountAmount] DECIMAL(18,2) NOT NULL DEFAULT 0,
    [createdAt] DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
    ...
    CONSTRAINT [CK_BOOKING_COMPENSATION_DISCOUNT_AMOUNT] CHECK ([compensationDiscountAmount] >= 0),
...
CREATE TABLE [CANCELLATION_COMPENSATION] (
    ...
    CONSTRAINT [CK_CANCELLATION_COMPENSATION_STATUS] CHECK ([status] IN ('ISSUED', 'PARTIALLY_USED', 'USED', 'EXPIRED', 'VOIDED')),
    CONSTRAINT [CK_CANCELLATION_COMPENSATION_EXPIRY] CHECK ([expiresAt] > [issuedAt]),
    CONSTRAINT [FK_CANCELLATION_COMPENSATION_BOOKING] FOREIGN KEY ([sourceBookingId]) REFERENCES [BOOKING]([bookingId]),
    CONSTRAINT [FK_CANCELLATION_COMPENSATION_SHOWTIME_CANCELLATION] FOREIGN KEY ([showtimeCancellationId]) REFERENCES [SHOWTIME_CANCELLATION]([showtimeCancellationId]),
    CONSTRAINT [FK_CANCELLATION_COMPENSATION_CUSTOMER_PROFILE] FOREIGN KEY ([customerProfileId]) REFERENCES [CUSTOMER_PROFILE]([customerProfileId])
);
GO

CREATE TABLE [COMPENSATION_TICKET] (
    [compensationTicketId] NVARCHAR(50) PRIMARY KEY,
    ...
    CONSTRAINT [CK_COMPENSATION_TICKET_STATUS] CHECK ([status] IN ('ISSUED', 'RESERVED', 'REDEEMED', 'EXPIRED', 'VOIDED')),
    CONSTRAINT [FK_COMPENSATION_TICKET_COMPENSATION] FOREIGN KEY ([cancellationCompensationId]) REFERENCES [CANCELLATION_COMPENSATION]([cancellationCompensationId]) ON DELETE CASCADE,
    CONSTRAINT [FK_COMPENSATION_TICKET_RESERVED_BOOKING] FOREIGN KEY ([reservedBookingId]) REFERENCES [BOOKING]([bookingId]),
    CONSTRAINT [FK_COMPENSATION_TICKET_RESERVED_BOOKING_SEAT] FOREIGN KEY ([reservedBookingSeatId]) REFERENCES [BOOKING_SEAT]([bookingSeatId])
);
GO

CREATE TABLE [COMPENSATION_COMBO] (
    [compensationComboId] NVARCHAR(50) PRIMARY KEY,
    [cancellationCompensationId] NVARCHAR(50) NOT NULL,
    [voucherCode] NVARCHAR(100) NOT NULL,
    ...
    CONSTRAINT [CK_COMPENSATION_COMBO_STATUS] CHECK ([status] IN ('ISSUED', 'REDEEMED', 'EXPIRED', 'VOIDED')),
    CONSTRAINT [FK_COMPENSATION_COMBO_COMPENSATION] FOREIGN KEY ([cancellationCompensationId]) REFERENCES [CANCELLATION_COMPENSATION]([cancellationCompensationId]) ON DELETE CASCADE,
    CONSTRAINT [FK_COMPENSATION_COMBO_CINEMA] FOREIGN KEY ([redeemedAtCinemaId]) REFERENCES [CINEMA]([cinemaId]),
    CONSTRAINT [FK_COMPENSATION_COMBO_STAFF_PROFILE] FOREIGN KEY ([redeemedByStaffProfileId]) REFERENCES [STAFF_PROFILE]([staffProfileId])
);
GO
...
CREATE TABLE [VOUCHER_USAGE] (
    ...
    [customerProfileId] NVARCHAR(50) NOT NULL,
    [bookingId] NVARCHAR(50) NOT NULL,
    [customerVoucherId] NVARCHAR(50) NULL,
    ...
    CONSTRAINT [FK_VOUCHER_USAGE_CUSTOMER_VOUCHER] FOREIGN KEY ([customerVoucherId]) REFERENCES [CUSTOMER_VOUCHER]([customerVoucherId]),
...
CREATE INDEX [IX_CANCELLATION_COMPENSATION_SHOWTIME_CANCELLATION] ON [CANCELLATION_COMPENSATION]([showtimeCancellationId]);
CREATE INDEX [IX_CANCELLATION_COMPENSATION_CUSTOMER_STATUS] ON [CANCELLATION_COMPENSATION]([customerProfileId], [status]);
CREATE INDEX [IX_COMPENSATION_TICKET_COMPENSATION] ON [COMPENSATION_TICKET]([cancellationCompensationId]);
CREATE INDEX [IX_COMPENSATION_TICKET_RESERVED_BOOKING] ON [COMPENSATION_TICKET]([reservedBookingId]);
...
CREATE UNIQUE INDEX [UX_VOUCHER_USAGE_ACTIVE_CUSTOMER_VOUCHER]
    ...
    WHERE [customerVoucherId] IS NOT NULL AND [usageStatus] <> 'CANCELLED';
